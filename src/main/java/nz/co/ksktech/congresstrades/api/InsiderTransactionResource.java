...
/**
 * Corporate-insider (SEC Form 3/4/5) transactions for a ticker, fetched live from
 * Finnhub. This is a <strong>Finnhub-only</strong> passthrough — it is not part of
 * the congressional dataset and is not persisted. Requires {@code FINNHUB_API_KEY}.
 *
 * <p>Research data only — NOT financial advice.</p>
 */
...
public class InsiderTransactionResource {
    ...
    @GET
    @Operation(summary = "Insider transactions for a ticker (Finnhub). "
            + "Required: ?symbol=TSLA. Optional: &from=2026-01-01&to=2026-06-05 (ISO dates).")
    public InsiderTransactionsResponseDto get(@QueryParam("symbol") String symbol,
                                              @QueryParam("from") String from,
                                              @QueryParam("to") String to) {
        if (symbol == null || symbol.isBlank()) {
            throw new IllegalArgumentException("Query parameter 'symbol' is required (e.g. ?symbol=TSLA).");
        }
        String token = appConfig.finnhub().apiKey().filter(k -> !k.isBlank())
