...
/**
 * Translates exceptions into the uniform {@link ErrorResponse} JSON envelope and
 * an appropriate HTTP status. Keeps stack traces out of API responses.
 */
@Provider
public class GlobalExceptionMapper implements ExceptionMapper<Throwable> {
    ...
    public Response toResponse(Throwable exception) {
        ...
        if (exception instanceof LlmUnavailableException) {
            ...
            return build(Response.Status.SERVICE_UNAVAILABLE, "Digest Unavailable", exception.getMessage());
        }
        ...
        if (exception instanceof WebApplicationException wae) {
            return build(Response.Status.fromStatusCode(wae.getResponse().getStatus()),
