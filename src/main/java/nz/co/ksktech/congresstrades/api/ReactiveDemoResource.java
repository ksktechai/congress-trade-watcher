...
/**
 * Reactive (Mutiny) variants of read endpoints, demonstrating {@code Uni<T>} and {@code Multi<T>}.
 *
 * <p>The rest of the app is intentionally blocking (Hibernate ORM Panache over JDBC). To use those
 * blocking repositories from a reactive endpoint without blocking the I/O event loop, the blocking
 * work is wrapped in a deferred supplier and offloaded to the worker pool via {@code
 * runSubscriptionOn(...)} — the idiomatic Mutiny pattern for bridging blocking code.
 */
...
public class ReactiveDemoResource {
  ...
  public ReactiveDemoResource(
      TradeRepository tradeRepository, WatchlistRepository watchlistRepository) {
  ...
  @Operation(
      summary =
          "Reactive trades by ticker — returns Uni<List<TradeDto>>; the blocking "
