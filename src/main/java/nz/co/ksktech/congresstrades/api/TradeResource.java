...
public class TradeResource {
    ...
    @GET
    @Transactional
    @Operation(summary = "List trades filtered by member, ticker, transaction type and date range "
            + "(from/to are ISO dates, e.g. 2026-05-01)")
    public List<TradeDto> list(@QueryParam("member") String member,
                               @QueryParam("ticker") String ticker,
                               @QueryParam("type") TransactionType type,
                               @QueryParam("from") String from,
                               @QueryParam("to") String to) {
        return tradeRepository.search(member, ticker, type, parseDate(from, "from"), parseDate(to, "to")).stream()
                .map(TradeDto::from)
                .toList();
    }
    ...
    private LocalDate parseDate(String value, String field) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Invalid '" + field + "' date '" + value + "'; expected ISO yyyy-MM-dd");
        }
    }
}
