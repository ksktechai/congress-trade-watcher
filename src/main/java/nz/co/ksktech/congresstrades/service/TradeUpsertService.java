...
/**
 * Source-agnostic, idempotent persistence for trades. Every ingestion source
 * normalises its raw records into a {@link NormalizedTrade} and calls
 * {@link #upsert}, so dedupe/upsert logic lives in exactly one place.
 *
 * <p>Idempotency is keyed on {@code sourceFilingId}: re-ingesting the same
 * disclosure updates the existing row instead of duplicating it.</p>
 */
@ApplicationScoped
public class TradeUpsertService {
    ...
    /**
     * Insert or update a single trade. Members are upserted by full name and
     * enriched with chamber/party/state when better information arrives. Skips
     * records that cannot be attributed (no member or no stable id).
     */
    @Transactional
    public void upsert(NormalizedTrade nt, IngestionResult result) {
        if (nt.memberName() == null || nt.memberName().isBlank()
                || nt.sourceFilingId() == null || nt.sourceFilingId().isBlank()) {
            return;
        }
        ...
        Member member = memberRepository.findByFullName(memberName)
                .orElseGet(() -> {
