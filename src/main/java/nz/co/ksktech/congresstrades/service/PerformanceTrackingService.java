 ...
 *
 * <p><strong>Honest limitation:</strong> the Finnhub free tier exposes only a current quote, not
 * historical daily closes, so a true "return since the disclosed trade date" cannot be computed
 * here. This service therefore reports the current quote and the day's move as a baseline, clearly
 * labelled, rather than fabricating a post-trade return. It exists to enrich research output, not
 * to score "performance" of a member's picks.
 */
@ApplicationScoped
public class PerformanceTrackingService {
  ...
  /**
   * Current price snapshot for a ticker, or {@code null} if the quote could not be retrieved
   * (caller decides how to degrade).
   */
  public PriceSnapshot snapshot(String ticker) {
    try {
      QuoteResponse quote =
          finnhubClient.getQuote(ticker.toUpperCase(), appConfig.finnhub().apiKey().orElse(""));
