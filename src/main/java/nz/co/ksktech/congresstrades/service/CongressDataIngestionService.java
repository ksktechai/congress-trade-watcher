...
/**
 * Ingests real STOCK Act disclosures from the free Congress Trading Monitor dataset
 * (congress.kadoa.com) — no API key required. For each active watchlist ticker it fetches that
 * ticker's filings and upserts them idempotently using the filing's stable {@code id}.
 *
 * <p>Each ticker is ingested in its own transaction so one failing symbol does not roll back the
 * rest of the run.
 */
@ApplicationScoped
public class CongressDataIngestionService {
  ...
  @Inject @RestClient CongressDataClient congressDataClient;
