...
/**
 * {@link LlmProvider} backed by a local Ollama server (OpenAI-compatible). Needs no API key, so it
 * is always "configured"; if Ollama is not running the call fails with a clear connection error
 * surfaced as the digest's 503.
 */
@ApplicationScoped
public class OllamaLlmProvider implements LlmProvider {
  ...
  public String generate(String systemPrompt, String userPrompt) {
    ...
    LOG.infof(
        "LLM via ollama (model=%s, finishReason=%s, chars=%d, tokens prompt/completion=%s/%s)",
        cfg.model(),
