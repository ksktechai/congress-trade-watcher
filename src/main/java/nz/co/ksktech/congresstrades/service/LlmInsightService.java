 ...
 *
 * <p><strong>Boundary:</strong> the model only <em>summarises and contextualises</em> data this app
 * has already derived. The system prompt forbids buy/sell recommendations and price predictions and
 * requires the data's limitations to be stated. The LLM never selects tickers or produces signals —
 * that is the job of the pure-code {@link SignalDetectionService}.
 */
@ApplicationScoped
public class LlmInsightService {
  ...
  @Inject Instance<LlmProvider> providers;

  @Inject AppConfig appConfig;

  /**
   * Builds the user prompt from computed data. Deterministic and side-effect free, so it can be
  ...
  public String buildUserPrompt(LocalDate date, List<Trade> trades, List<Signal> signals) {
    ...
    if (trades.isEmpty()) {
      ...
      Map<String, List<Trade>> byTicker =
          trades.stream()
      ...
      for (Map.Entry<String, List<Trade>> entry : byTicker.entrySet()) {
        ...
        for (Trade t : entry.getValue()) {
          ...
          sb.append("  - ")
              .append(member)
              .append(" ")
              .append(t.transactionType)
              .append(" range $")
              .append(t.amountRangeLow)
              .append("-$")
              .append(t.amountRangeHigh)
              .append(", traded ")
              .append(t.transactionDate)
              .append(", disclosed ")
              .append(t.disclosureDate)
              .append(" (")
              .append(t.daysToDisclose)
              .append(" days later)\n");
        }
      }
    }
  ...
  /**
   * Calls the configured provider ({@code watcher.llm.provider}) to produce the narrative. Throws
   * {@link LlmUnavailableException} on any failure (missing key, timeout, open circuit, unknown
   * provider) so callers can degrade.
   */
  ...
  private LlmProvider resolveProvider(String providerId) {
    ...
    throw new LlmUnavailableException(
        "Unknown LLM provider '"
            + providerId
            + "'. Supported: gemini, openrouter, ollama, anthropic.");
  }
}
