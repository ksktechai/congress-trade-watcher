 ...
 * persists them idempotently via {@link TradeUpsertService}.
 *
 * <p>Idempotency: Finnhub exposes no stable filing id, so we derive a
 * deterministic {@code sourceFilingId} from the immutable fields of a disclosure.
 * Finnhub's congressional-trading endpoint is premium, so this source may return
 * HTTP 403 on a free key — the alternative {@link CongressDataIngestionService}
 * is free and is the default ingestion source.</p>
 */
@ApplicationScoped
