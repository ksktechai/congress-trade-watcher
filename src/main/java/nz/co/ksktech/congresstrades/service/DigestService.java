...
/**
 * Builds the daily digest: gathers the look-back trades, ensures signals are fresh, asks {@link
 * LlmInsightService} to narrate them, and assembles the response with the mandatory disclaimer.
 *
 * <p>The result is cached per date ({@code @CacheResult}) so repeated calls in a day do not re-bill
 * the Anthropic API.
 */
@ApplicationScoped
public class DigestService {
  ...
  /**
   * Evicts all cached digests so the next call re-runs the pipeline (and the LLM). Used by {@code
   * ?refresh=true} on the digest endpoint.
   */
  ...
  public DigestResponse dailyDigest(LocalDate date) {
    LOG.infof(
        "DIGEST build start for %s (cache miss; look-back=%d days)",
    ...
    LOG.infof(
        "DIGEST step 3/3 — narrative generated (%d chars); caching for %s",
        narrative.length(), date);

    List<SignalDto> signalDtos = signals.stream().map(SignalDto::from).toList();
    return new DigestResponse(
        date, narrative, trades.size(), signalDtos, Disclaimers.NOT_FINANCIAL_ADVICE);
  }
