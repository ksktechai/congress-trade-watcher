...
/**
 * {@link LlmProvider} backed by the Google Gemini {@code generateContent} API. The system prompt is
 * sent as Gemini's {@code systemInstruction}.
 */
@ApplicationScoped
public class GeminiLlmProvider implements LlmProvider {
  ...
  @Inject @RestClient GeminiClient geminiClient;
  ...
  public String generate(String systemPrompt, String userPrompt) {
    ...
    GeminiGenerateRequest request =
        GeminiGenerateRequest.of(
            systemPrompt,
            userPrompt,
            appConfig.gemini().maxTokens(),
            appConfig.gemini().thinkingBudget());

    LOG.infof(
        "LLM REQUEST  → gemini POST /v1beta/models/%s:generateContent "
    ...
    GeminiGenerateResponse response =
        geminiClient.generateContent(model, appConfig.gemini().apiKey().orElse(""), request);
    long ms = System.currentTimeMillis() - start;

    String text = response.firstText();
    GeminiGenerateResponse.UsageMetadata usage = response.usageMetadata();
    LOG.infof(
        "LLM RESPONSE ← gemini (%dms, finishReason=%s, chars=%d, tokens prompt/out/thoughts=%s/%s/%s)",
        ms,
