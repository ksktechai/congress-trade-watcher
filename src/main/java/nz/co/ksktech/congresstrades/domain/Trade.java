 ...
 *
 * <p>Amounts are stored as a low/high <em>range</em> because the STOCK Act only requires members to
 * disclose a bracket (e.g. $1,001 - $15,000), never an exact figure. {@code sourceFilingId} is
 * unique so ingestion is idempotent.
 */
@Entity
@Table(
    name = "trades",
...
public class Trade extends PanacheEntityBase {
  ...
  /**
   * Midpoint of the disclosed amount range, used by signal/analysis code as a single representative
   * figure. Returns {@code null} when no range is known.
   */
  public BigDecimal estimatedAmount() {
    ...
    return amountRangeLow.add(amountRangeHigh).divide(BigDecimal.valueOf(2));
  }

  /**
   * Recomputes {@link #daysToDisclose} from the transaction and disclosure dates. Safe to call
   * repeatedly.
   */
