package nz.co.ksktech.congresstrades.logging;
...
/**
 * Shared helpers for request/response logging: secret masking in URLs, the
 * correlation-id MDC key, and bounded (CPU- and memory-capped) body previews.
 */
public final class LogSupport {

    /** MDC key surfaced in the log format as {@code [%X{correlationId}]}. */
    ...
    private static final Set<String> SECRET_PARAMS = Set.of(
            "token", "apikey", "api_key", "key", "password", "secret", "access_token");

    private static volatile int payloadMaxChars = -1;

    private LogSupport() {
    }

    /** Max characters of a request/response body to log; 0 disables body logging. */
    public static int payloadMaxChars() {
        int cached = payloadMaxChars;
        if (cached < 0) {
            cached = ConfigProvider.getConfig()
                    .getOptionalValue("watcher.logging.payload-max-chars", Integer.class)
                    .orElse(2000);
            payloadMaxChars = cached;
        }
        return cached;
    }

    /**
     * Masks the values of sensitive query parameters (e.g. {@code token},
     * {@code apikey}) so credentials never reach the logs.
     */
    public static String maskUrl(String url) {
        ...
        String masked = Arrays.stream(url.substring(q + 1).split("&"))
    ...
    /**
     * Serialises {@code entity} to JSON, capping both the work done and the bytes
     * kept at {@link #payloadMaxChars()}.
     */
    public static String preview(ObjectMapper mapper, Object entity) {
        int cap = payloadMaxChars();
    ...
    /**
     * A {@link Writer} that keeps at most {@code cap} characters and aborts
     * serialisation (via {@link Stop}) once full, so large payloads are never
     * fully built just to be truncated.
     */
