 ...
 *
 * <p>Like the other external clients, the API key is passed per-request (here as the {@code
 * X-goog-api-key} header) and the method carries the fault-tolerance ring. Used <strong>only to
 * narrate</strong> already-computed data — never to pick stocks or predict prices.
 *
 * <p>Base URL is configured via {@code quarkus.rest-client.gemini-api.url}.
 */
