...
/**
 * Typed client for a local <a href="https://ollama.com">Ollama</a> server's OpenAI-compatible
 * chat-completions endpoint ({@code http://localhost:11434/v1/chat/completions}).
 *
 * <p>Fully local: no API key, no rate limits, nothing leaves the machine. The timeout is generous
 * because local models can be slow. Base URL via {@code quarkus.rest-client.ollama-api.url}.
 */
