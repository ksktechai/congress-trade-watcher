...
/**
 * Logs every outbound call made by the {@code @RegisterRestClient} clients
 * (Finnhub, Gemini, Anthropic, congress-data) as a request/response pair, with
 * the same correlation id as the inbound request (so all lines group together)
 ...
 * </ul>
 *
 * <p>Auth headers (x-api-key, X-goog-api-key, Authorization) are never logged.</p>
 */
@Provider
public class ExternalClientLoggingFilter implements ClientRequestFilter, ClientResponseFilter {
    ...
    /**
     * Reads up to the configured cap from the response stream for logging, then
     * restores the full stream (buffered head + untouched tail) so the client can
     * still deserialise the entity.
     */
