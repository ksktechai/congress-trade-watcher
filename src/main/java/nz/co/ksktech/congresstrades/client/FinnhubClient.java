 ...
 *
 * <p>This is the canonical example of an external client in this project: a
 * {@code @RegisterRestClient} interface, the API token passed as a {@code @QueryParam} (Finnhub's
 * real auth model), and a fault-tolerance ring ({@code @Retry} + {@code @Timeout} +
 * {@code @CircuitBreaker}) on every method.
 *
 * <p>Base URL is configured via {@code quarkus.rest-client.finnhub-api.url}.
 */
...
public interface FinnhubClient {
  ...
  CongressionalTradingResponse getCongressionalTrades(
      @QueryParam("symbol") String symbol, @QueryParam("token") String token);
  ...
  /**
   * Corporate-insider (SEC Form 3/4/5) transactions for a ticker, optionally bounded by from/to
   * dates (ISO {@code yyyy-MM-dd}). Null date params are omitted from the request.
   */
  @GET
  ...
  @CircuitBreaker(requestVolumeThreshold = 8, failureRatio = 0.5, delay = 10000)
  InsiderTransactionsResponse getInsiderTransactions(
