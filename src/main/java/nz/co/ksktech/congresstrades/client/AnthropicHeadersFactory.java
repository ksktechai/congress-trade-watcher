...
/**
 * Injects the Anthropic auth and protocol headers on every request:
 * {@code x-api-key} (from the {@code ANTHROPIC_API_KEY} env var, via
 * {@link AppConfig}), {@code anthropic-version}, and {@code content-type}.
 *
 * <p>The API key is read from configuration, never hard-coded, so no secret
 * touches source control.</p>
 */
@ApplicationScoped
