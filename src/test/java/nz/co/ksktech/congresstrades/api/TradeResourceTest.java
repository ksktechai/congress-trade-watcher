...
/**
 * End-to-end API tests over the full stack: Testcontainers Postgres (via Quarkus Dev Services) +
 * Flyway-managed schema + WireMock-stubbed external APIs. No real Finnhub or Anthropic calls
 * happen.
 */
@QuarkusTest
...
class TradeResourceTest {
  ...
  void memberTrades_returnsThatMembersTrades() {
    given()
        .pathParam("name", "Jane Representative")
        .when()
        .get("/api/v1/members/{name}/trades")
        .then()
        .statusCode(200)
        .body("size()", greaterThan(0))
