...
/**
 * Verifies the prompt is built from computed data and the LLM response is parsed. The real APIs are
 * never called — WireMock stands in for both Gemini (the default provider) and Anthropic.
 */
@QuarkusTest
@QuarkusTestResource(WireMockTestResource.class)
class LlmInsightServiceTest {

  @Inject LlmInsightService llmInsightService;

  @Inject AnthropicLlmProvider anthropicLlmProvider;

  @Inject OpenRouterLlmProvider openRouterLlmProvider;

  @Inject OllamaLlmProvider ollamaLlmProvider;

  @Test
  void buildUserPrompt_includesComputedTradesAndSignals() {
    String prompt =
        llmInsightService.buildUserPrompt(
  ...
  void generateNarrative_usesDefaultGeminiProviderAndParsesResponse() {
    String narrative =
        llmInsightService.generateNarrative(
            LocalDate.of(2026, 6, 4), sampleTrades(), sampleSignals());
  ...
  void openRouterProvider_callsOpenRouterAndParsesResponse() {
    String text =
        openRouterLlmProvider.generate(
            LlmInsightService.SYSTEM_PROMPT,
            llmInsightService.buildUserPrompt(
                LocalDate.of(2026, 6, 4), sampleTrades(), sampleSignals()));

    assertTrue(
        text.startsWith("MOCK OPENROUTER DIGEST"), "should return the parsed OpenRouter text");
    WireMockTestResource.server()
        .verify(
  ...
  void systemPrompt_forbidsRecommendations() {
    assertTrue(
        LlmInsightService.SYSTEM_PROMPT.contains("Do NOT give buy, sell, or hold recommendations"));
  ...
  private List<Signal> sampleSignals() {
    return List.of(
        new Signal(
            SignalType.CLUSTER,
            "AAPL",
            "3 distinct members purchased AAPL within a 14-day window.",
            3.0,
            List.of()));
  }
}
